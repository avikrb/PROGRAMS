...
list_or = []
dictionary = {}
i = 0#using i for index value. it loops along with quote, so data_list[5] would later on be equal as i = 5
for quote in data_list:
    ...
    for each_word in words:
        ...
        dictionary.setdefault(each_word,[]).append(i)#setdefault intializes the value for the key and if key exists it does nothing
        dictionary[each_word] = list(set(dictionary[each_word]))#get rid of duplicates and convert back to a list
        dictionary[each_word].sort()#useful for sorted printout later on
    i = i+1
...
print ("Performing", search_type.upper(), "for", query)
if search_type == "and":
    ...
    for each_element in dict_list_1:#loop through all values in dict_list_1 and prinout. dict_list_1 now has all the "and" values of the query
        print("-->> Found at: ", each_element, data_list[each_element][:80])
    ...
    for every_value in list_or:
        print ("-->> Found at: ", every_value, data_list[every_value][:80])
        
else:#finally, if user enters just one string
    for value in dictionary[query[0]]:
        print ("-->> Found at: ", value, data_list[value][:80])
...
'''
******OBSERVATION******
The execution time when using the dictionary to preprocess the data_list was seen to quicken the search. This was because
each word from data_list was mapped with the indices of their occurances throughout the data_list, which allowed the search program to just compare the indices
and print out the result. Examples: "and" operation and "or" operation were carried out on "sheep","flower" using both programs and the execution time were checked. The
execution time was much lesser in the second program where dictionary was implemented. The output results are posted on github and are labelled as:
"time diff hw2 part2", "time diff2 hw2 part2"

WORK BY: AVIK RANJAN BHATTARAI
'''
