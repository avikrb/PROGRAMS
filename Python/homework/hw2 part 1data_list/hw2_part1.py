...
query=input("query:")
query = query.strip(" ") #get rid of the front and rear spaces
...
if search_type == "or":
    ...
    for i,quote in enumerate(data_list):
        count = 0
        index0 = 1000 #for finding the indexes of the query words
        for item in query:#useful if user uses multiple query words
            if item in quote:#one keyword at a time
                count = count + 1 
                index1 = quote.index(item) #for finding the index 
                if index1 < index0:#swapping the higher and lower index
                    index0 = index1#index0 has the lowest index of the user's query words
        if count == len(query):#if count value matches with the length of the contents in the query list
            print("Found:", i, "...", quote[index0:index1+50], "...")#print from 1st query to atleast one other query
